...
func (c *EtherscanClient) GetAllTransactionHashes(address string) ([]string, error) {
	return getAllTypesOfTransactionHash(address, string(c.network.Name),
		labeledGetter{"normal", withAnyReturn(c.client.NormalTxByAddress)},
		labeledGetter{"internal", withAnyReturn(c.client.InternalTxByAddress)},
		labeledGetter{"erc20", withAnyReturn(withAnyContractAddress(c.client.ERC20Transfers))},
		labeledGetter{"erc721", withAnyReturn(withAnyContractAddress(c.client.ERC721Transfers))},
		labeledGetter{"erc1155", withAnyReturn(withAnyContractAddress(c.client.ERC1155Transfers))},
	)
}
...
func getAllTypesOfTransactionHash(
	address string,
	networkName string,
	txGetters ...labeledGetter,
) ([]string, error) {
	hashLists := make([][]string, len(txGetters))

	for i, getter := range txGetters {
		hashes, err := getTransactionHashes(address, networkName, getter.label, getter.getTxs)
		if err != nil {
...
func getTransactionHashes(
	...
	if networkName == "fantom" && label == "erc1155" {
		// FantomScan doesn't support this endpoint, so if there are any, they are
